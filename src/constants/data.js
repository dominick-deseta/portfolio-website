export const properties = [
    // {
    //     id: 1,
    //     thumbnail: "samples/man-on-a-escalator",
    //     title: "AHAnonymous",
    //     type: "Android Application",
    //     tools: ["Android Studio", "Firebase"],
    //     screenshots: ["test/phone1", "test/phone2", "test/phone3", "test/phone4"],
    //     githubUrl:"",
    //     description:"AH Anonymous test description AH Anonymous test description AH Anonymous test description AH Anonymous test description lorem ipsum dolor compiled successfully you can now view portfolio-website in the browser let me know if you need more help with this or if you have any other question! setting line height to a value less thn 1 will reduce the spacing between lines. you can experiment with different values to ahieve the desired spacing for your wrapped span element."
    // },
    {
        id: 2,
        thumbnail: "hack/loading",
        title: "HackIllinois App",
        type: "Android Application",
        tools: ["Android Studio", "Node.js", "Kotlin"],
        screenshots: ["hack/welcome", "hack/home", "hack/profile", "hack/scanner", "hack/card", "hack/loading"],
        githubUrl:"https://github.com/HackIllinois/android",
        description:"HackIllinois test description HackIllinois test description HackIllinois test description HackIllinois test description HackIllinois test description "
    },
    {
        id: 3,
        thumbnail: "calendar/calendar1",
        title: "Google Calendar Widget",
        type: "Android Widget",
        tools: ["Android Studio", "Google Calendar API", "Kotlin"],
        screenshots: ["calendar/calendar1", "calendar/calendar2"],
        githubUrl:"https://github.com/dominick-deseta/calendar-widget",
        description:"Calendar Widget test description Calendar Widget test description Calendar Widget test description Calendar Widget test description Calendar Widget test description Calendar Widget test description "
    },
    // {
    //     id: 4,
    //     thumbnail: "samples/balloons",
    //     title: "Crime Data Visualizer",
    //     type: "React Website",
    //     tools: ["React", "Node.js", "MySQL", "GCP"],        
    //     screenshots: ["test/1", "test/2", "test/3", "test/4"],
    //     githubUrl:"",
    //     description:"Crime Data visualizer test description Crime Data visualizer test description Crime Data visualizer test description Crime Data visualizer test description Crime Data visualizer test description "
    // },
    {
        id: 5,
        thumbnail: "fractal/render1",
        title: "Fractal Renderer",
        type: "Java Swing Application",
        tools: ["Java Swing"],
        screenshots: ["fractal/render1", "fractal/render2", "fractal/render3", "fractal/render4", "fractal/render5", "fractal/render6"],
        githubUrl:"https://github.com/dominick-deseta/fractal-renderer",
        description:"Discover the hidden beauty of math! This application allows the user to explore the infinitely detailed Mandelbrot set. The user may click and drag their mouse to magnify or shift the camera's viewpoint. Each point rendered is a complex number that is iterated through a specific formula. If the number diverges to infinity, its point is assigned a color. If it remains bounded, the point is part of the Mandelbrot set and is colored black. With controls that change the computation of color, you can create some amazing imagery."
    },
    // {
    //     id: 6,
    //     thumbnail: "samples/man-on-a-street",
    //     title: "Form Processor",
    //     type: "React Website",
    //     tools: ["React"],
    //     screenshots: ["test/1", "test/2", "test/3", "test/4"],
    //     githubUrl:"",
    //     description:"Form Processor test description Form Processor test description Form Processor test description Form Processor test description Form Processor test description Form Processor test description Form Processor test description "
    // },
    {
        id: 7,
        thumbnail: "",
        title: "Online Portfolio",
        type: "The website you're on now!",
        tools: [],
        screenshots: [],
        githubUrl:"https://github.com/dominick-deseta/portfolio-website",
        description:""
    }
  ];